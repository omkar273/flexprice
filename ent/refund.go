...
func (*Refund) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case refund.FieldCreatedAt, refund.FieldUpdatedAt, refund.FieldInitiatedAt, refund.FieldClaimedAt, refund.FieldSucceededAt, refund.FieldFailedAt, refund.FieldCancelledAt:
			values[i] = new(sql.NullTime)
		case refund.ForeignKeys[0]: // payment_refunds
			values[i] = new(sql.NullString)
		default:
...
func (r *Refund) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case refund.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field payment_refunds", values[i])
			} else if value.Valid {
				r.payment_refunds = new(string)
				*r.payment_refunds = value.String
			}
		default:
