...
type UsageAnalyticItem struct {
	...
	TotalReportingUsage  string                             `json:"total_reporting_usage"` // Empty string when feature has no reporting unit; otherwise the value in reporting units
	ReportingUnit        *types.ReportingUnit               `json:"reporting_unit,omitempty"` // Present when total_reporting_usage is set (unit_singular, unit_plural, conversion_rate)
	TotalCost            decimal.Decimal                    `json:"total_cost" swaggertype:"string"`
