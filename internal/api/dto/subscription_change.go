...
type SubscriptionChangeRequest struct {
	...
	AdjustmentAmount *decimal.Decimal `json:"-"`
}
...
func (r *SubscriptionChangeRequest) Validate() error {
	...
	// Scheduled end-of-period changes do not use client adjustment (see product spec).
	if r.AdjustmentAmount != nil && r.ChangeAt != nil && lo.FromPtr(r.ChangeAt) == types.ScheduleTypePeriodEnd {
		return ierr.NewError("adjustment cannot be set when change_at is period_end").
			WithHint("Use immediate change if you need a custom proration adjustment.").
			Mark(ierr.ErrValidation)
	}
