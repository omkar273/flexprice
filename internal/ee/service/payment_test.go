...
func (s *PaymentServiceSuite) TestCreatePaymentLink_AutomaticTax() {
	ctx := types.SetEnvironmentID(s.GetContext(), "test-env-id")
	...
	s.Run("enabled_on_stripe_payment_link_persists_to_gateway_metadata", func() {
		req := baseReq()
		req.IdempotencyKey = "idem_automatic_tax_alone"
		req.GatewayOptions = &dto.PaymentGatewayOptions{
			Stripe: &dto.StripePaymentGatewayOptions{AutomaticTaxEnabled: lo.ToPtr(true)},
		}
		...
		stored, err := s.GetStores().PaymentRepo.Get(ctx, resp.ID)
		s.Require().NoError(err)
		s.Equal("true", stored.GatewayMetadata["automatic_tax_enabled"])
	})
	...
	s.Run("both_tax_flags_together_persist_independently", func() {
		req := baseReq()
		req.IdempotencyKey = "idem_both_tax_flags"
		req.GatewayOptions = &dto.PaymentGatewayOptions{
			Stripe: &dto.StripePaymentGatewayOptions{
				TaxIDCollectionEnabled: lo.ToPtr(true),
				AutomaticTaxEnabled:    lo.ToPtr(true),
			},
		}
