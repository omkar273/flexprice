...
const invoiceBulkBatchSize = 500

// CreateBulk creates multiple invoices in a single transaction with batching.
// Duplicate idempotency keys are silently skipped (constraint errors are ignored per-batch).
func (r *invoiceRepository) CreateBulk(ctx context.Context, invoices []*domainInvoice.Invoice) error {
	if len(invoices) == 0 {
		return nil
	}
	...
	return r.client.WithTx(ctx, func(ctx context.Context) error {
		...
		builders := make([]*ent.InvoiceCreate, len(invoices))
		for i, inv := range invoices {
			if inv.EnvironmentID == "" {
				inv.EnvironmentID = types.GetEnvironmentID(ctx)
			}

			builders[i] = client.Invoice.Create().
				SetID(inv.ID).
				SetTenantID(inv.TenantID).
				SetCustomerID(inv.CustomerID).
				SetNillableSubscriptionID(inv.SubscriptionID).
		...
		for i := 0; i < len(builders); i += invoiceBulkBatchSize {
			end := i + invoiceBulkBatchSize
			if end > len(builders) {
				end = len(builders)
			}
			if _, err := client.Invoice.CreateBulk(builders[i:end]...).Save(ctx); err != nil {
				return ierr.WithError(err).
					WithHint("failed to bulk create invoices").
					WithReportableDetails(map[string]any{
						"batch_start": i,
						"batch_end":   end,
						"total":       len(builders),
